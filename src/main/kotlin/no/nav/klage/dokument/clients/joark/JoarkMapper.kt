...
class JoarkMapper {
    ...
    fun createPartialJournalpostWithoutDocuments(
        ...
        val kanal =
            if (journalfoeringData.journalpostType == JournalpostType.INNGAAENDE) {
        ...
        val partialJournalpostWithoutDocuments =
            JournalpostPartial(
                ...
                tilleggsopplysninger =
                    journalfoeringData.tilleggsopplysning?.let {
                        listOf(
                            Tilleggsopplysning(
                                nokkel = it.key,
                                verdi = it.value,
    ...
    private fun createJournalpostAvsenderMottager(avsenderMottaker: AvsenderMottaker): JournalpostAvsenderMottaker =
        if (avsenderMottaker.partId != null) {
            JournalpostAvsenderMottaker(
                id = avsenderMottaker.partId!!.value,
                idType =
                    if (avsenderMottaker.partId!!.type == PartIdType.PERSON) {
    ...
    private fun createBruker(journalfoeringData: JournalfoeringData): Bruker =
        Bruker(
            id = journalfoeringData.sakenGjelder.value,
            idType =
